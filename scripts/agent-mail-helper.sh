...
GLOBAL_AGENT_NAME_FILE="$AGENT_FLYWHEEL_ROOT/pids/${SAFE_PANE}.agent-name"
...
EOF
    ...
    # Ensure sender is registered in target project
...
send_message() {
    ...
    for entry in "${groups[@]}"; do
        ...
        send_message_with_project "$project_root" "$recipients_csv" "$subject" "$body" "$importance"
    done

    # Auto-notify recipients via terminal (scan all projects)
    local my_name=$(whoami_agent)
    IFS=',' read -ra RECIPIENTS <<< "$to_agents"
    for recipient in "${RECIPIENTS[@]}"; do
        recipient=$(echo "$recipient" | xargs)
        [ -z "$recipient" ] && continue
        # Use environment variable or parent of current project, then fall back to $HOME
        ...
        while IFS= read -r identity_file; do
            ...
            if [ -f "$identity_file" ]; then
                ...
                if [ "$mail_name" = "$recipient" ]; then
                    local pane=$(jq -r '.pane' "$identity_file")
                    tmux send-keys -t "$pane" "📨 NEW MAIL from $my_name" 2>/dev/null
                    sleep 0.3
                    tmux send-keys -t "$pane" C-m 2>/dev/null
                    echo "  Notified $recipient at $pane"
                    break
                fi
            fi
        done < <(find "$search_root" -maxdepth 5 -path "*/panes/*.identity" -type f 2>/dev/null)
    done
}
...
EOF
    ...
    if [ -z "$messages" ] || [ "$messages" = "null" ] || [ "$messages" = "[]" ]; then
        ...
        echo "$response" | jq -r '.result.structuredContent.result[] | "[\(.importance)] From: \(.from) | \(.subject)\n\(.body_md)\n---"'
    fi
}
